...
function build_with_DAFL() {
    CC="/fuzzer/DAFL_energy/afl-clang-fast"
    CXX="/fuzzer/DAFL_energy/afl-clang-fast++"
    ...
    for TARG in "${@:2}"; do
        ...
        for BUG_NAME in "${str_array[@]:1}"; do
            ...
            build_target $1 $CC $CXX "$BIT_OPT" 2>&1 | tee /benchmark/build_log/$BIN_NAME-$BUG_NAME.txt

            ### copy results
            copy_build_result $1 $BIN_NAME $BUG_NAME "DAFL-energy"
            rm -rf RUNDIR-$1
        done
    done
...
mkdir -p /benchmark/bin/DAFL-energy
