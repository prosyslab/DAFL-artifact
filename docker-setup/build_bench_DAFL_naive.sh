...
function build_with_DAFL() {
    ...
    for TARG in "${@:2}"; do
        ...
        for BUG_NAME in "${str_array[@]:1}"; do
            export DAFL_SELECTIVE_COV="/benchmark/DAFL-input-naive/inst-targ/$BIN_NAME/$BUG_NAME"
            export DAFL_DFG_SCORE="/benchmark/DAFL-input-naive/dfg/$BIN_NAME/$BUG_NAME"
            build_target $1 $CC $CXX "-fsanitize=integer-divide-by-zero $BIT_OPT" 2>&1 | tee /benchmark/build_log/$BIN_NAME-$BUG_NAME.txt

            ### copy results
            copy_build_result $1 $BIN_NAME $BUG_NAME "DAFL-naive"
            rm -rf RUNDIR-$1
        done
    done
